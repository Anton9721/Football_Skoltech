...
def run_benchmark(
    ...
    if csv_path.exists():
        ...
        print(f"Загружен {csv_path}: {len(existing)} строк, уникальных (game, model): {len(done_pairs)}")
    else:
        existing = pd.DataFrame()
        done_pairs = set()

    todo = [
        (g, m) for g in games for m in all_model_names
        if (g, m) not in done_pairs
    ]
    if not todo:
    ...
    for game in todo_games:
        ...
        needed_finetuned  = {k: v for k, v in finetuned_configs.items() if k in needed_models}

        emb = {}
        if needed_pretrained:
            emb.update(extract_all_models(
                df_match=df_match,
                game_id=game,
                device=device,
        ...
        if needed_finetuned:
            emb.update(extract_all_finetuned(
                df_match=df_match,
                game_id=game,
                device=device,
